...
> extends Middleware<
	...
	TState,
	TState & {
		request: ApiRequestValidatorState<TBody, TPath, TQuery>;
	},
	TRes,
	TRes
> {
	constructor(
		private readonly options: ApiRequestValidatorOptions<TBody, TPath, TQuery>
	) {
		super('request');
	}
    ...
    apply(
        ...
        next: NextFunction<
            TEvent,
            TState & { request: ApiRequestValidatorState<TBody, TPath, TQuery> },
            TRes
        >
    ): PromiseOrValue<TRes> {
		const { bodySchema, pathSchema, querySchema } = this.options;
		const { event } = request;

		let parsedPath: unknown = undefined;
		if (pathSchema !== undefined) {
			try {
				parsedPath = pathSchema.parse(event.pathParameters ?? {}) as z.infer<
				typeof pathSchema
			>;
			} catch (err) {
		...
		if (querySchema !== undefined) {
			try {
				...
				if (err instanceof z.ZodError) {
					throw new ValidationError(`Received invalid query parameters`, {
						cause: err,
						detail: {
		...
		if (bodySchema !== undefined) {
			...
			try {
				...
				throw new ValidationError(`Invalid JSON body format`, {
					cause: err,
					detail: { message: getKey(err, 'message') },
				});
			}
			...
			try {
				...
				if (err instanceof z.ZodError) {
					throw new ValidationError(`Validation failed for message body`, {
						cause: err,
...
> => new ApiRequestValidator<TEvent, TState, TRes, TBody, TPath, TQuery>(options);
